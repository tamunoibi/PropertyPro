...
export default class UserController {
  static async createAccount(req, res) {
    try {
      ...
      const {
        first_name, last_name, password, phoneNumber, address, is_admin,
      } = req.body;
      const userData = {
        email, first_name, last_name, password, phoneNumber, address, is_admin,
      };
      const user = create(userData);
      const { id, is_admin: adminStatus } = user;
      const token = await generateToken({ id, adminStatus });
  ...
  static async signinUser(req, res) {
    try {
      ...
      if (!user) return res.status(401).send({ status: 'error', error: 'Invalid username or password' });
